...
namespace pinocchio
{
  ...
  : public JointDataBase<JointDataEllipsoidTpl<_Scalar, _Options>>
  {
  ...
  : public JointModelBase<JointModelEllipsoidTpl<_Scalar, _Options>>
  {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    typedef JointEllipsoidTpl<_Scalar, _Options> JointDerived;
    ...
    explicit JointModelEllipsoidTpl(
      const Scalar & a,
      const Scalar & b,
      const Scalar & c)
    : radius_a(a), radius_b(b), radius_c(c)
    {
    }
      ...
      const
    {
      ...
      computeMotionSubspace(s0, c0, s1, c1, s2, c2, dndotx_dqdot1, dndoty_dqdot0, dndoty_dqdot1, dndotz_dqdot0, dndotz_dqdot1, data);
      
      // Velocity part
      data.v.toVector().noalias() = data.S.matrix() * data.joint_v;

      computeMotionSubspaceDerivative(s0, c0, s1, c1, s2, c2, dndotx_dqdot1, dndoty_dqdot0, dndoty_dqdot1, dndotz_dqdot0, dndotz_dqdot1, data);
  ...
  void computeMotionSubspace(
    const Scalar & s0, const Scalar & c0,
    const Scalar & s1, const Scalar & c1,
    const Scalar & s2, const Scalar & c2,
